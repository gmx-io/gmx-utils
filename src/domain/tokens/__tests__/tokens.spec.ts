...
describe("convertToContractPrice", () => {
  it("divides price by 10^decimals", () => {
    expect(convertToContractPrice(10000n, 2)).toBe(
      10000n / expandDecimals(1, 2)
    );
  });
});
...
describe("convertToContractTokenPrices", () => {
  it("returns min and max contract prices", () => {
    const result = convertToContractTokenPrices(
      { minPrice: 1000n, maxPrice: 2000n },
      2
    );
...
describe("convertToTokenAmount", () => {
  ...
  it("converts usd to token amount", () => {
    expect(convertToTokenAmount(1000n, 2, 100n)).toBe(
      (1000n * expandDecimals(1, 2)) / 100n
    );
  });
});
...
describe("convertToUsd", () => {
  ...
  it("converts token amount to usd", () => {
    expect(convertToUsd(1000n, 2, 100n)).toBe(
      (1000n * 100n) / expandDecimals(1, 2)
    );
  });
});
  ...
  it("returns wrapped if convertTo=wrapped and token isNative", () => {
    const tokensData = {
      "0xnative": {
        address: "0xnative",
        isNative: true,
        wrappedAddress: "0xwrap",
      },
      "0xwrap": { address: "0xwrap", isWrapped: true },
    } as unknown as TokensData;
    expect(getTokenData(tokensData, "0xnative", "wrapped")).toEqual(
      tokensData["0xwrap"]
    );
  });
});
...
describe("getTokensRatioByAmounts", () => {
  it("returns ratio of two token amounts", () => {
    ...
    expect(result.ratio).toBe(
      (((1000n * PRECISION) / expandDecimals(1, 2)) * PRECISION) /
        ((500n * PRECISION) / expandDecimals(1, 2))
    );
  });
});
