...
import { getContractMarketPrices } from "domain/pricing/contractPrice";
...
describe("getMarketIndexName", () => {
  ...
  it("returns prefix + baseSymbol/symbol + /USD", () => {
    ...
    expect(getMarketIndexName({ indexToken: eth, isSpotOnly: false })).toBe(
      "ETH/USD"
    );

    const pepe = getToken("PEPE");
    expect(getMarketIndexName({ indexToken: pepe, isSpotOnly: false })).toBe(
      "kPEPE/USD"
    );
  });
});
...
describe("getMarketPoolName", () => {
  it("returns single token symbol if long and short are the same", () => {
    ...
    expect(getMarketPoolName({ longToken: token, shortToken: token })).toBe(
      "ETH-ETH"
    );
  });
...
describe("getTokenPoolType", () => {
  it("returns 'long' for single-token markets if matches address", () => {
    const token = getToken("ETH");
    expect(
      getTokenPoolType({ longToken: token, shortToken: token }, token.address)
    ).toBe("long");
  });
  ...
  it("returns 'short' for shortToken match", () => {
    ...
    const shortToken = getToken("USDC");
    expect(
      getTokenPoolType({ longToken, shortToken }, shortToken.address)
    ).toBe("short");
  });
});
...
describe("getPoolUsdWithoutPnl", () => {
  const marketInfo = {
    ...
    longToken: {
      decimals: 18,
      prices: {
        minPrice: expandDecimals(5, 30),
        maxPrice: expandDecimals(15, 30),
      },
    },
    shortToken: {
      decimals: 18,
      prices: {
        minPrice: expandDecimals(2, 30),
        maxPrice: expandDecimals(4, 30),
      },
    },
  } as MarketInfo;

  it("calculates poolUsd for isLong = true", () => {
    expect(getPoolUsdWithoutPnl(marketInfo, true, "minPrice")).toBe(
      5000000000000n
    );
    expect(getPoolUsdWithoutPnl(marketInfo, true, "maxPrice")).toBe(
      15000000000000n
    );
  });

  it("calculates poolUsd for isLong = false", () => {
    expect(getPoolUsdWithoutPnl(marketInfo, false, "minPrice")).toBe(
      2000000000000n
    );
    expect(getPoolUsdWithoutPnl(marketInfo, false, "maxPrice")).toBe(
      4000000000000n
    );
  });
});
...
describe("getMaxLeverageByMinCollateralFactor", () => {
  ...
  it("returns correct leverage for a given factor", () => {
    expect(getMaxLeverageByMinCollateralFactor(1000000000000000000n)).toBe(
      10000000000000000
    );
  });
});
...
describe("getOppositeCollateral", () => {
  ...
  it("returns shortToken if token is long", () => {
    expect(
      getOppositeCollateral(marketInfo, marketInfo.longToken.address)
    ).toEqual(marketInfo.shortToken);
  });
...
describe("getAvailableUsdLiquidityForCollateral", () => {
  it("returns poolUsd if isSpotOnly", () => {
    const marketInfo = {
      ...
      indexToken: {
        ...getToken("ETH"),
        prices: {
          minPrice: expandDecimals(10, 18),
          maxPrice: expandDecimals(15, 18),
        },
      },
      longToken: {
        decimals: 18,
        prices: {
          minPrice: expandDecimals(1, 18),
          maxPrice: expandDecimals(2, 18),
        },
      },
    } as MarketInfo;
  ...
  it("calculates liquidity if not spot only", () => {
    ...
    expect(getAvailableUsdLiquidityForCollateral(marketInfo, true)).toBe(
      expandDecimals(35, 30)
    );
  });
});
  ...
  it("returns 0n if openInterest is 0", () => {
    const marketInfo = {
      indexToken: {
        decimals: 18,
        prices: {
          minPrice: expandDecimals(1000, 18),
          maxPrice: expandDecimals(2000, 18),
        },
      },
      longInterestUsd: 0n,
...
describe("getOpenInterestUsd", () => {
  it("returns longInterestUsd for isLong", () => {
    expect(
      getOpenInterestUsd(
        { longInterestUsd: 1234n, shortInterestUsd: 9999n } as MarketInfo,
        true
      )
    ).toBe(1234n);
  });
...
describe("getPriceForPnl", () => {
  it("uses maxPrice for long when maximize=true", () => {
    expect(
      getPriceForPnl({ minPrice: 1000n, maxPrice: 2000n }, true, true)
    ).toBe(2000n);
  });

  it("uses maxPrice for short when maximize=false", () => {
    expect(
      getPriceForPnl({ minPrice: 1000n, maxPrice: 2000n }, false, false)
    ).toBe(2000n);
  });
});
