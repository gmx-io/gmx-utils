...
import { getPriceImpactForPosition } from "domain/executionFee";
...
import { convertToUsd, getIsEquivalentTokens } from "domain/tokens/utils";
import { bigMath } from "lib/bigmath";
import { expandDecimals, USD_DECIMALS } from "lib/numbers";

vi.mock("domain/markets/utils", () => ({
  ...vi.importActual("domain/markets/utils"),
  getMarketPnl: vi.fn(),
  getPoolUsdWithoutPnl: vi.fn(),
  getCappedPoolPnl: vi.fn(),
}));

vi.mock("domain/tokens/utils", () => ({
  ...vi.importActual("domain/tokens/utils"),
  convertToUsd: vi.fn(),
  getIsEquivalentTokens: vi.fn(),
}));

vi.mock("domain/executionFee", async () => {
  const actual = await vi.importActual<typeof import("domain/executionFee")>(
    "domain/executionFee"
  );
  return {
    ...actual,
...
describe("getPositionKey", () => {
  it("returns key in expected format", () => {
    expect(getPositionKey("0xabc", "0xmarket", "0xcollateral", true)).toBe(
      "0xabc:0xmarket:0xcollateral:true"
    );
  });
});
...
describe("getEntryPrice", () => {
  it("returns undefined if sizeInTokens <= 0", () => {
    ...
    expect(
      getEntryPrice({ sizeInUsd: 1000n, sizeInTokens: 0n, indexToken: token })
    ).toBeUndefined();
  });

  it("returns mulDiv of sizeInUsd if sizeInTokens > 0", () => {
    const token = { decimals: 2 } as Token;
    const result = getEntryPrice({
      sizeInUsd: 1000n,
      sizeInTokens: 100n,
      indexToken: token,
    });
...
describe("getPositionValueUsd", () => {
  it("uses convertToUsd under the hood", () => {
    ...
    const token = { decimals: 18 } as Token;
    const result = getPositionValueUsd({
      indexToken: token,
      sizeInTokens: 100n,
      markPrice: 10n,
    });
...
describe("getPositionPnlUsd", () => {
  const marketInfo = {
    indexToken: {},
    maxPositionImpactFactorForLiquidations: 2n,
  } as MarketInfo;
...
describe("getLiquidationPrice", () => {
  beforeEach(() => {
    ...
    (getPriceImpactForPosition as Mock).mockReturnValue({
      priceImpactDeltaUsd: -100n,
