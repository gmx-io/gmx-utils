...
import { mockMarketsInfoData } from "domain/markets/__tests__/mock";
import { MarketInfo } from "domain/markets/types";
import { buildMarketsAdjacencyGraph } from "domain/swap/buildMarketsAdjacencyGraph";
import { findSwapPathsBetweenTokens } from "domain/swap/findSwapPathsBetweenTokens";
import {
...
} from "domain/swap/swapRouting";
import { mockTokensData, usdToToken } from "domain/tokens/__tests__/mock";
...
const marketsConfigMap = Object.fromEntries(
  Object.entries(baseMarketsInfoData).map(([key, market]) => [
    key,
    {
      ...
      enabled: !market.isDisabled,
    },
  ])
);
const marketAdjacencyGraph = buildMarketsAdjacencyGraph(marketsConfigMap);
  ...
  it("selects SPOT [USDC-DAI] path for USDC->DAI swap when pool has excess of DAI", () => {
    const allPoolsBalanced = Object.fromEntries(
      marketKeys.map((marketKey) => [
        ...
        {
          marketKey,
          longPoolAmount: usdToToken(
            1000,
            baseMarketsInfoData[marketKey].longToken
          ),
          shortPoolAmount: usdToToken(
            1000,
            baseMarketsInfoData[marketKey].shortToken
          ),
        },
      ])
    );
    ...
    const tokenSwapPaths = getTokenSwapPathsForTokenPair(
      swapPaths,
      "USDC",
      "DAI"
    );
    ...
    const allPoolsBalanced = Object.fromEntries(
      marketKeys.map((marketKey) => [
        marketKey,
        {
          ...
          maxLongPoolAmount: usdToToken(
            1_000_000,
            baseMarketsInfoData[marketKey].longToken
          ),
          maxShortPoolAmount: usdToToken(
            1_000_000,
            baseMarketsInfoData[marketKey].shortToken
          ),
