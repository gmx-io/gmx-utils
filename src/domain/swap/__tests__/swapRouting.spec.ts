...
import { mockTokensData } from "domain/tokens/__tests__/mock";
    ...
    const result = getBestSwapPath({
      ...
      networkEstimator: (usdIn, swapCount) => {
        if (swapCount === 1) {
          return {
            networkYield: 0.99,
            usdOut: bigMath.mulDiv(usdIn, 99n, 100n),
          };
        } else if (swapCount === 2) {
          return {
            networkYield: 0.97,
            usdOut: bigMath.mulDiv(usdIn, 97n, 100n),
          };
        }
    ...
    const result = getBestSwapPath({
      ...
      networkEstimator: (usdIn, swapCount) => {
        if (swapCount === 1) {
          ...
          return {
            networkYield: 0.98,
            usdOut: bigMath.mulDiv(usdIn, 98n, 100n),
          };
        }
