...
import { mockMarketsInfoData as createMockMarketsInfoData } from "domain/markets/__tests__/mock";
import type { MarketsInfoData } from "domain/markets/types";
import {
  getSwapPathOutputAddresses,
  getSwapPathStats,
} from "domain/swap/swapStats";
...
describe("getSwapPathStats", () => {
  ...
  const testMarketsInfoData = createMockMarketsInfoData(
    tokensData,
    marketKeys,
    {
      "ETH-WETH-USDC": {
  ...
  it("accumulates fees and price impact across multiple hops", () => {
    ...
    const totalFeesFromSteps = result.swapSteps.reduce(
      (sum, step) => sum + step.swapFeeUsd,
      0n
    );
    expect(result.totalSwapFeeUsd).toBe(totalFeesFromSteps);

    // Total price impact should be sum of individual step impacts
    const totalImpactFromSteps = result.swapSteps.reduce(
      (sum, step) => sum + step.priceImpactDeltaUsd,
      0n
    );
