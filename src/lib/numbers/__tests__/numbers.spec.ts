...
import { BASIS_POINTS_DIVISOR_BIGINT, USD_DECIMALS } from "lib/numbers";
import {
  applyFactor,
  basisPointsToFloat,
  bigintToNumber,
...
describe("toBigNumberWithDecimals", () => {
  it("should convert string to big number with decimals", () => {
    ...
    expect(toBigNumberWithDecimals("1", PRECISION_DECIMALS)).toBe(
      1000000000000000000000000000000n
    );
    expect(toBigNumberWithDecimals("123.456", PRECISION_DECIMALS)).toBe(
      123456000000000000000000000000000n
    );
    expect(toBigNumberWithDecimals("123.456789", PRECISION_DECIMALS)).toBe(
      123456789000000000000000000000000n
    );
    expect(toBigNumberWithDecimals("-1.5", PRECISION_DECIMALS)).toBe(
      -1500000000000000000000000000000n
    );
    expect(toBigNumberWithDecimals("0.000001", PRECISION_DECIMALS)).toBe(
      1000000000000000000000000n
    );
  });

  it("should handle strings with more decimals than token decimals parameter", () => {
    expect(toBigNumberWithDecimals("0.123456789012345678901234567890", 5)).toBe(
      12345n
    );
  });
  ...
  it("should handle cases with different token decimals ", () => {
    expect(
      toBigNumberWithDecimals("0.1234567890123456789012345678901", 18)
    ).toBe(123456789012345678n);
  ...
  it("should be compatible with formatAmount", () => {
    expect(
      formatAmount(
        toBigNumberWithDecimals("123.456", PRECISION_DECIMALS),
        USD_DECIMALS,
        3
      )
    ).toBe("123.456");
    expect(
      formatAmount(
        toBigNumberWithDecimals("0.789", PRECISION_DECIMALS),
        USD_DECIMALS,
        2
      )
    ).toBe("0.79");
...
describe("roundWithDecimals", () => {
  it("should round small numbers correctly", () => {
    expect(
      roundWithDecimals(
        toBigNumberWithDecimals("0.0000001", PRECISION_DECIMALS),
        {
  ...
  it("should round large numbers correctly", () => {
    expect(
      roundWithDecimals(
        toBigNumberWithDecimals("499.999999", PRECISION_DECIMALS),
        {
          decimals: PRECISION_DECIMALS,
          displayDecimals: 5,
        }
  ...
  it("should handle complex rounding cases", () => {
    ...
    expect(
      roundWithDecimals(
        toBigNumberWithDecimals("123.456789", PRECISION_DECIMALS),
        {
          decimals: PRECISION_DECIMALS,
          displayDecimals: 4,
        }
  ...
  it("should round numbers with different token decimals correctly", () => {
    ...
    for (const tokenDecimals of differentTokenDecimals) {
      expect(
        roundWithDecimals(
          toBigNumberWithDecimals("123.456789", tokenDecimals),
          {
...
describe("formatAmountHuman", () => {
  ...
  it("negative", () => {
    ...
    expect(formatAmountHuman(-1n * ONE_USD * 1000n, USD_DECIMALS)).toBe(
      "-1.0k"
    );
    expect(formatAmountHuman(-1n * ONE_USD * 1000000n, USD_DECIMALS)).toBe(
      "-1.0m"
    );
  });
  ...
  it("should display dollar sign", () => {
    ...
    expect(formatAmountHuman(-1n * ONE_USD, USD_DECIMALS, true)).toBe(
      "-$\u200a1.0"
    );
  });

  it("should display decimals", () => {
    expect(formatAmountHuman(ONE_USD * 1000n, USD_DECIMALS, false, 2)).toBe(
      "1.00k"
    );
    expect(formatAmountHuman(ONE_USD * 1500000n, USD_DECIMALS, false, 2)).toBe(
      "1.50m"
    );
    expect(formatAmountHuman(ONE_USD * 1000n, USD_DECIMALS, false, 0)).toBe(
      "1k"
    );
    expect(formatAmountHuman(ONE_USD * 1500000n, USD_DECIMALS, false, 0)).toBe(
      "2m"
    );
  });
});
...
describe("formatBalanceAmount", () => {
  ...
  it("should display balance amount with symbol", () => {
    expect(formatBalanceAmount(ONE_USD, USD_DECIMALS, "USDC")).toBe(
      "1.0000 USDC"
    );

    expect(
      formatBalanceAmount(0n, USD_DECIMALS, "USDC", { showZero: true })
    ).toBe("0.0000 USDC");
    expect(
      formatBalanceAmount(0n, USD_DECIMALS, "USDC", { showZero: false })
    ).toBe("-");
  });
...
describe("formatPercentage", () => {
  ...
  it("should handle basis points (bps) formatting", () => {
    expect(
      formatPercentage(
        toBigNumberWithDecimals("1", PERCENT_PRECISION_DECIMALS),
        { bps: false, displayDecimals: 4 }
      )
    ).toBe("1.0000%");
    expect(
      formatPercentage(
        toBigNumberWithDecimals("0.999", PERCENT_PRECISION_DECIMALS),
        { bps: false, displayDecimals: 5 }
      )
    ).toBe("0.99900%");
  });
});
