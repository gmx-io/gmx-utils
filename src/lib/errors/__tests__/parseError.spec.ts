...
import { parseError, ErrorLike } from "lib/errors";
import { TxErrorType } from "lib/errors/transactionsErrors";
    ...
    describe("error masking", () => {
      it("should mask URLs in error groups", () => {
        const error = new Error(
          "Failed to fetch data from https://api.example.com:8080/v1/data?param=123"
        );
        const result = parseError(error);
      ...
      it("should mask numbers in error groups", () => {
        const error = new Error(
          "Transaction failed with gas 123456 at block 789012"
        );
        const result = parseError(error);
      ...
      it("should mask both URLs and numbers in error groups", () => {
        const error = new Error(
          "Failed  https://api.example.com/v1/tx/123456 with status 404 and error code E123"
        );
        const result = parseError(error);
