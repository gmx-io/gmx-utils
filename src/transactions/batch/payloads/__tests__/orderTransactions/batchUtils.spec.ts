...
import { expandDecimals, parseValue, USD_DECIMALS } from "lib/numbers";
import {
...
function buildMultiIncreaseBatch(
  paramsList: (typeof commonMarketIncreaseParams)[]
) {
  return {
...
describe("Batch Utils", () => {
  describe("getIsEmptyBatch", () => {
    ...
    it("multiple orders batch with one empty order", () => {
      ...
      const nonEmptyOrder = buildIncreaseOrderPayload(
        commonMarketIncreaseParams
      );
      const batch = {
    ...
    it("multiple non-empty orders batch", () => {
      const batch = buildMultiIncreaseBatch([
        commonMarketIncreaseParams,
        commonMarketIncreaseParams,
      ]);
  ...
  describe("getBatchExternalCalls", () => {
    it("combines external calls from multiple orders", () => {
      ...
      expect(result).toEqual({
        ...
        sendAmounts: [
          parseValue("1", WETH.decimals)!,
          parseValue("1000", USDC.decimals)!,
        ],
        externalCallTargets: [
  ...
  describe("getBatchTotalExecutionFee", () => {
    it("calculates total execution fee including top-ups", () => {
      ...
      batch.updateOrderParams = [
        {
          updatePayload: {
            executionFeeTopUp: parseValue("0.05", WETH.decimals)!,
          },
        },
      ] as any;
  ...
  describe("getBatchExternalSwapGasLimit", () => {
    it("sums gas limits from external swap quotes", () => {
      ...
      expect(result).toBe(
        quote1.txnData.estimatedGas + quote2.txnData.estimatedGas
      );
    });
  });
});
