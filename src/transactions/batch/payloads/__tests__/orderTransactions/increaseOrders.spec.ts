...
import { mockExternalSwap } from "domain/externalSwap/__tests__/mock";
import { OrderType } from "domain/orders/types";
import { ContractPrice } from "domain/pricing/types";
import { MOCK_GAS_PRICE } from "domain/tokens/__tests__/mock";
import { ERC20Address } from "domain/tokens/types";
import { decodeTwapUiFeeReceiver } from "domain/uiFeeReceiver/uiFeeReceiver";
import { MaxUint256, parseValue, USD_DECIMALS } from "lib/numbers";
import {
...
describe("Increase Order Payloads", () => {
  ...
  const REFERRAL_CODE =
    "0xf2742351cc0eca941ff90bf489789ee6169cbeacfdd38eba60012218fac1b7e5";

  const USDC = getTokenBySymbol(CHAIN_ID, "USDC");
  const WETH = getWrappedToken(CHAIN_ID);
  const ETH_MARKET =
    MARKETS[CHAIN_ID]["0x70d95587d40A2caf56bd97485aB3Eec10Bee6336"];
  ...
  describe("buildIncreaseOrderPayload", () => {
        ...
        orderPayload: {
          ...
          numbers: {
            ...
            acceptablePrice: parseValue(
              "1206",
              USD_DECIMALS - WETH.decimals
            )! as ContractPrice, // $1200 + 0.5% slippage
    ...
    it("TWAP Increase Long Pay with Native Token", () => {
      ...
      const totalDurationInSeconds =
        twapParams.duration.hours * 3600 + twapParams.duration.minutes * 60;
      const startTime = Math.ceil(Date.now() / 1000);
      const intervalInSeconds =
        totalDurationInSeconds / (twapParams.numberOfParts - 1);

      const expectedOrders = Array.from(
        { length: twapParams.numberOfParts },
        (_, i) => {
          const validFromTime = BigInt(
            Math.floor(startTime + intervalInSeconds * i)
          );
          ...
          return {
            ...
            tokenTransfersParams: {
              ...
              tokenTransfers: [
                {
                  amount:
                    EXECUTION_FEE_AMOUNT / 4n +
                    parseValue("0.25", WETH.decimals)!,
                  destination: ORDER_VAULT_ADDRESS,
                  tokenAddress: NATIVE_TOKEN_ADDRESS,
                },
              ],
              ...
              swapPath: [ETH_MARKET.marketTokenAddress],
              value:
                EXECUTION_FEE_AMOUNT / 4n + parseValue("0.25", WETH.decimals)!,
              externalCalls: undefined,
            },
          } satisfies CreateOrderTxnParams<IncreasePositionOrderParams>;
        }
      ...
      expect(
        expectedOrders.every((co) =>
          getIsTwapOrderPayload(co.orderPayload as CreateOrderPayload)
        )
      ).toBe(true);
    });
          ...
          return {
            ...
            tokenTransfersParams: {
              ...
              tokenTransfers: [
                {
                  amount:
                    EXECUTION_FEE_AMOUNT / 4n + params.payTokenAmount / 4n,
                  destination: ORDER_VAULT_ADDRESS,
                  tokenAddress: NATIVE_TOKEN_ADDRESS,
                },
              ],
