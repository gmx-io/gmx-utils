import { defineChain, zeroAddress, encodeFunctionData, decodeFunctionResult, withRetry, createPublicClient, http } from 'viem';
...
import { BOTANIX, SOURCE_BSC_MAINNET, SOURCE_BASE_MAINNET, SOURCE_SEPOLIA, SOURCE_OPTIMISM_SEPOLIA, ARBITRUM_SEPOLIA as ARBITRUM_SEPOLIA$1, AVALANCHE as AVALANCHE$1, ARBITRUM as ARBITRUM$1, AVALANCHE_FUJI as AVALANCHE_FUJI$1 } from './chainIds.js';
import { AVALANCHE, ARBITRUM, AVALANCHE_FUJI, BOTANIX as BOTANIX$1, ARBITRUM_SEPOLIA } from './chains.js';
...
var VIEM_CHAIN_BY_CHAIN_ID = {
  [AVALANCHE_FUJI$1]: avalancheFuji,
  [ARBITRUM$1]: arbitrum,
  [AVALANCHE$1]: avalanche,
  [ARBITRUM_SEPOLIA$1]: arbitrumSepolia,
  [BOTANIX]: botanix,
...
// src/transactions/simulation/simulation.ts
...
function isSimulationPassed(errorData) {
  return (0, errors_exports.isContractError)(errorData, errors_exports.CustomErrorName.EndOfOracleSimulation);
}
  ...
  try {
    await withRetry(
      ...
      },
      {
        ...
        delay: 200,
        shouldRetry: ({ error }) => {
          const errorData = (0, errors_exports.parseError)(error);
    ...
    const errorData = (0, errors_exports.parseError)(txnError);
    const isPassed = errorData && isSimulationPassed(errorData);
    const shouldIgnoreExpressNativeTokenBalance = errorData?.txErrorType === errors_exports.TxErrorType.NotEnoughFunds && p.isExpress;
    if (isPassed || shouldIgnoreExpressNativeTokenBalance) {
      return;
    } else {
      throw (0, errors_exports.extendError)(txnError, {
...
function getSimulationPrices(chainId, tokensData, overrides) {
  ...
  for (const address of tokenAddresses) {
    ...
    const convertedAddress = convertTokenAddress(chainId, address);
