import { defineChain, zeroAddress, keccak256, stringToBytes, encodeAbiParameters, size, encodeFunctionData, zeroHash, recoverTypedDataAddress } from 'viem';
...
import 'viem/chains';
import { BOTANIX, AVALANCHE as AVALANCHE$1, ARBITRUM as ARBITRUM$1, ARBITRUM_SEPOLIA as ARBITRUM_SEPOLIA$1, SOURCE_BSC_MAINNET, SOURCE_BASE_MAINNET, SOURCE_SEPOLIA, SOURCE_OPTIMISM_SEPOLIA, AVALANCHE_FUJI as AVALANCHE_FUJI$1 } from './chainIds.js';
import { BOTANIX as BOTANIX$1, ARBITRUM_SEPOLIA, AVALANCHE_FUJI, AVALANCHE, ARBITRUM } from './chains.js';
...
({
  ...
  [BOTANIX$1]: [getTokenBySymbol(BOTANIX$1, "pBTC").address]
});
...
var CHAIN_IDS = [ARBITRUM, AVALANCHE, AVALANCHE_FUJI, BOTANIX$1, ARBITRUM_SEPOLIA];
...
function getWrappedToken2(chainId) {
...
// src/domain/executionFee/executionFee.ts
...
function getIsSubaccountActionsExceeded(subaccount, requiredActions) {
  return getRemainingSubaccountActions(subaccount) < bigMath2.max(1n, BigInt(requiredActions));
}
...
function getBatchTotalExecutionFee({
  ...
  let gasLimit = 0n;
  const wnt = getByKey(tokensData, getWrappedToken2(chainId).address);
  if (!wnt) {
    return void 0;
  }
...
async function estimateExpressParams({
  ...
  if (estimationMethod === "estimateGas" && provider) {
    ...
    try {
      ...
      const extendedError = (0, errors_exports.extendError)(error, {
        data: {
          estimationMethod
        }
      });
