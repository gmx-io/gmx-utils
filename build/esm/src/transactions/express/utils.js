import { defineChain, zeroAddress, encodeAbiParameters, keccak256 } from 'viem';
...
// src/domain/externalSwap/utils.ts
...
function getOracleParams({
  ...
  const uniqTokenAddresses = Array.from(
    new Set(
      tokenAddresses.map(
        (tokenAddress) => convertTokenAddress(chainId, tokenAddress)
      )
    )
  );
