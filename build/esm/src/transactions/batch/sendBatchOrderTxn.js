import { defineChain, zeroAddress, keccak256, stringToBytes, encodeAbiParameters, withRetry, encodeFunctionData, encodePacked, zeroHash, recoverTypedDataAddress } from 'viem';
import { BOTANIX, ARBITRUM_SEPOLIA, AVALANCHE_FUJI, AVALANCHE, ARBITRUM } from './chains.js';
import * as types_star from './types.js';
import { TaskState, TxnEventBuilder, OrderType } from './types.js';
...
import 'viem/chains';
import { BOTANIX as BOTANIX$1, AVALANCHE as AVALANCHE$1, ARBITRUM as ARBITRUM$1, ARBITRUM_SEPOLIA as ARBITRUM_SEPOLIA$1, SOURCE_BSC_MAINNET, SOURCE_BASE_MAINNET, SOURCE_SEPOLIA, SOURCE_OPTIMISM_SEPOLIA, AVALANCHE_FUJI as AVALANCHE_FUJI$1 } from './chainIds.js';
import 'lodash/uniq';
import './ArbitrumNodeInterface.js';
import './ClaimHandler.js';
import './CustomErrors.js';
import './DataStore.js';
...
import { getGelatoRelayRouterDomain, hashRelayParams } from './relayParamsUtils.js';
...
hashString("POSITION_IMPACT_FACTOR");
hashString(
...
hashString("POSITION_FEE_FACTOR");
hashString("SWAP_IMPACT_FACTOR");
hashString(
  "SWAP_IMPACT_EXPONENT_FACTOR"
);
hashString("SWAP_FEE_FACTOR");
hashString("ATOMIC_SWAP_FEE_FACTOR");
hashString(
...
hashString("POOL_AMOUNT");
hashString(
  "MAX_POOL_USD_FOR_DEPOSIT"
);
hashString("MAX_POOL_AMOUNT");
hashString("RESERVE_FACTOR");
hashString(
  "OPEN_INTEREST_RESERVE_FACTOR"
);
hashString("MAX_OPEN_INTEREST");
hashString("NONCE");
...
hashString("MIN_COLLATERAL_FACTOR");
...
hashString("VIRTUAL_TOKEN_ID");
hashString("VIRTUAL_MARKET_ID");
hashString(
...
// src/lib/transactions/callRelayTransaction.ts
  ...
  try {
    ...
    if (maxFeePerGas !== void 0) {
      ...
      if (block.baseFeePerGas !== void 0 && block.baseFeePerGas !== null) {
        ...
        return {
          maxFeePerGas: bigMath2.max(maxFeePerGas, calculatedMaxFeePerGas),
...
// src/transactions/batch/payloads/orderTransactions.ts
...
function updateExpressOrdersAddresses(addresses) {
  return {
    ...
    uiFeeReceiver: setUiFeeReceiverIsExpress(addresses.uiFeeReceiver)
  };
}
async function validateSignature({
  ...
  try {
    ...
    if (!isValid) {
      throw (0, errors_exports.extendError)(new Error("Signature validation failed"), {
        data: {
          recoveredAddress,
          expectedAccount,
          signature
    ...
    (0, metrics_exports.emitMetricEvent)({
...
var signerAddressError = "Invalid receiver address";
function isTriggerDecreaseOrderType(orderType) {
  return [5 /* LimitDecrease */, 6 /* StopLossDecrease */].includes(
    orderType
  );
}
var DEFAULT_RUN_SIMULATION = () => Promise.resolve(void 0);
async function sendBatchOrderTxn({
  ...
  const eventBuilder = new TxnEventBuilder2({
    expressParams,
    batchParams,
    signer
  });
...
var makeBatchOrderSimulation = async ({
  chainId,
  signer,
  isGmxAccount,
  provider,
  ...
  try {
    if (getIsInvalidBatchReceiver(batchParams, signer.address)) {
      throw (0, errors_exports.extendError)(new Error(signerAddressError), {
    ...
    if (expressParams?.subaccount && expressParams?.subaccountValidations && !expressParams.subaccountValidations.isValid) {
      ...
      throw (0, errors_exports.extendError)(new Error("Invalid subaccount"), {
        data: {
    ...
    if (expressParams && expressParams.gasPaymentValidations.isOutGasTokenBalance) {
      throw (0, errors_exports.extendError)(new Error("Out of gas token balance"), {
        data: {
    ...
    if (expressParams && expressParams.gasPaymentValidations.needGasPaymentTokenApproval) {
      throw (0, errors_exports.extendError)(new Error("Need gas payment token approval"), {
        data: {
          gasPaymentTokenAmount: expressParams.gasPaymentParams.gasPaymentTokenAmount,
          gasPaymentTokenAddress: expressParams.gasPaymentParams.gasPaymentTokenAddress
        }
