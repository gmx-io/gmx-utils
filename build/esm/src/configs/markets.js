...
function getMarketByLabel(chainId, label) {
  ...
  const indexToken = getTokenBySymbol(
    chainId,
    fixTokenSymbolFromMarketLabel(chainId, indexSymbolRaw)
  );
  ...
  const market = Object.values(marketsByAddress).find(
    (market2) => market2.longTokenAddress === longToken.address && market2.shortTokenAddress === shortToken.address && market2.indexTokenAddress === indexToken.address
  );
  if (!market) {
...
export { MARKETS, SWAP_GRAPH_MAX_MARKETS_PER_TOKEN, fixTokenSymbolFromMarketLabel, getMarketByLabel };
