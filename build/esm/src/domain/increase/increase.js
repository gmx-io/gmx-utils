...
import { buildSwapStrategy, buildReverseSwapStrategy } from './buildSwapStrategy.js';
...
function getAmountByRatio(p) {
  ...
  const swapSlippageAmount = allowedSwapSlippageBps !== void 0 ? bigMath2.mulDiv(
    amount,
    allowedSwapSlippageBps,
    BASIS_POINTS_DIVISOR_BIGINT
  ) : 0n;
...
function getSwapAmountsByToValueDefault(p) {
  ...
  {
    const adjustedUsdIn = swapPathStats.usdOut > 0 ? bigMath2.mulDiv(baseUsdIn, usdOut, swapPathStats.usdOut) : 0n;
...
// src/domain/increase/increase.ts
function getIncreasePositionAmounts(p) {
  ...
  if (strategy === "leverageByCollateral" && leverage !== void 0 && initialCollateralAmount !== void 0 && initialCollateralAmount > 0) {
    ...
    const baseSizeDeltaUsd = bigMath2.mulDiv(
      baseCollateralUsd,
      leverage,
      BASIS_POINTS_DIVISOR_BIGINT
    );
    ...
    values.sizeDeltaUsd = bigMath2.mulDiv(
  ...
  if (isLimit) {
    if (!isSetAcceptablePriceImpactEnabled || limitOrderType === 8 /* StopIncrease */) {
...
function leverageBySizeValues({
  ...
  const collateralDeltaUsd = bigMath2.mulDiv(
    sizeDeltaUsd,
    BASIS_POINTS_DIVISOR_BIGINT,
    leverage
  );
