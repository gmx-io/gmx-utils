...
import { DecreasePositionSwapType } from './types.js';
...
function getMarketPnl(marketInfo, isLong, forMaxPoolValue) {
  ...
  const price = getPriceForPnl(marketInfo.indexToken.prices, isLong);
...
function getPositionPnlUsd(p) {
  ...
  const poolPnl = getMarketPnl(marketInfo, isLong);
  const poolUsd = getPoolUsdWithoutPnl(marketInfo, isLong);
...
function getMinCollateralFactorForPosition(position, openInterestDelta) {
  ...
  let minCollateralFactor = bigMath2.mulDiv(
    openInterest,
...
function getAcceptablePriceInfo(p) {
  ...
  if (maxNegativePriceImpactBps !== void 0 && maxNegativePriceImpactBps > 0) {
    let priceDelta = bigMath2.mulDiv(
      indexPrice,
      maxNegativePriceImpactBps,
      BASIS_POINTS_DIVISOR_BIGINT
    );
...
function getAcceptablePriceByPriceImpact(p) {
  ...
  const acceptablePrice = bigMath2.mulDiv(
    indexPrice,
...
function getDefaultAcceptablePriceImpactBps(p) {
  ...
  if (baseAcceptablePriceValues.acceptablePriceDeltaBps < 0) {
    return bigMath2.abs(baseAcceptablePriceValues.acceptablePriceDeltaBps) + BigInt(acceptablePriceImapctBuffer);
  }
  return BigInt(acceptablePriceImapctBuffer);
}
...
function getMarkPrice(p) {
  ...
  const shouldUseMaxPrice = getShouldUseMaxPrice2(isIncrease, isLong);
  return shouldUseMaxPrice ? prices.maxPrice : prices.minPrice;
}
function getShouldUseMaxPrice2(isIncrease, isLong) {
  return !isLong;
}
...
function getDecreasePositionAmounts(p) {
  ...
  const isTrigger = orderType !== void 0;
  if (orderType) {
    values.triggerPrice = triggerPrice;
    values.indexPrice = triggerPrice ?? markPrice;
    values.collateralPrice = getIsEquivalentTokens(indexToken, collateralToken) ? triggerPrice ?? markPrice : collateralToken.prices.minPrice;
  ...
  if (!position || position.sizeInUsd <= 0 || position.sizeInTokens <= 0) {
    ...
    const positionFeeInfo2 = getPositionFee(
      marketInfo,
      values.sizeDeltaUsd,
      values.balanceWasImproved,
      userReferralInfo
    );
    values.positionFeeUsd = positionFeeInfo2.positionFeeUsd;
    values.feeDiscountUsd = positionFeeInfo2.discountUsd;
    values.uiFeeUsd = applyFactor(values.sizeDeltaUsd, uiFeeFactor);
    const totalFeesUsd2 = getTotalFeesUsdForDecrease({
    ...
    values.payedOutputUsd = totalFeesUsd2;
    return values;
  }
  ...
  if (keepLeverage) {
    estimatedCollateralDeltaUsd = bigMath2.mulDiv(
      values.sizeDeltaUsd,
  ...
  if (values.isFullClose) {
    ...
    if (position.isLong) {
      ...
      values.sizeDeltaInTokens = bigMath2.mulDiv(
  ...
  values.realizedPnl = bigMath2.mulDiv(
    values.estimatedPnl,
    values.sizeDeltaInTokens,
    position.sizeInTokens
  );
  values.realizedPnlPercentage = estimatedCollateralUsd !== 0n ? getBasisPoints(values.realizedPnl, estimatedCollateralUsd) : 0n;
  values.estimatedPnlPercentage = estimatedCollateralUsd !== 0n ? getBasisPoints(values.estimatedPnl, estimatedCollateralUsd) : 0n;
  applyAcceptablePrice({
    position,
    marketInfo,
    isLong,
  ...
  if (values.isFullClose) {
  ...
  } else if (keepLeverage && position.sizeInUsd > 0 && estimatedCollateralUsd > 0 && payedInfo.remainingCollateralAmount > 0) {
     ...
     */
    leverageWithoutPnl !== void 0 && leverageWithoutPnl !== 0n ? remainingCollateralUsd - bigMath2.mulDiv(
      nextSizeInUsd,
      BASIS_POINTS_DIVISOR_BIGINT,
      leverageWithoutPnl
    ) : 0n;
...
function getIsFullClose(p) {
  ...
  const realizedPnl = bigMath2.mulDiv(
    estimatedPnl,
    sizeDeltaUsd,
    position.sizeInUsd
  );
  ...
  if (realizedPnl < 0) {
    ...
    let minCollateralFactor = isLong ? marketInfo.minCollateralFactorForOpenInterestLong : marketInfo.minCollateralFactorForOpenInterestShort;
    const minCollateralFactorForMarket = marketInfo.minCollateralFactor;
    if (minCollateralFactorForMarket > minCollateralFactor) {
      minCollateralFactor = minCollateralFactorForMarket;
    }
    ...
    if (!willCollateralBeSufficient) {
      if (estimatedRemainingCollateralUsd + estimatedRemainingPnl < minCollateralUsd || position.sizeInUsd - sizeDeltaUsd < minPositionSizeUsd) {
        return true;
      }
    }
  }
...
function applyAcceptablePrice(p) {
  ...
  if (isTrigger) {
    if (!isSetAcceptablePriceImpactEnabled || values.triggerOrderType === 6 /* StopLossDecrease */) {
...
function getTotalFeesUsdForDecrease({
  positionFeeUsd,
  borrowingFeeUsd,
  fundingFeeUsd,
  swapProfitFeeUsd,
  ...
  const negativePriceImpactUsd = totalPendingImpactDeltaUsd < 0 ? bigMath2.abs(totalPendingImpactDeltaUsd) : 0n;
  const negativePnlUsd = pnlUsd < 0 ? bigMath2.abs(pnlUsd) : 0n;
  const totalFeesUsd = positionFeeUsd + borrowingFeeUsd + fundingFeeUsd + swapProfitFeeUsd + swapUiFeeUsd + uiFeeUsd + negativePnlUsd + negativePriceImpactUsd;
  return totalFeesUsd;
}
function getNextPositionValuesForDecreaseTrade(p) {
  ...
  let nextCollateralUsd = existingPosition ? existingPosition.collateralUsd - collateralDeltaUsd - payedRemainingCollateralUsd : 0n;
  if (nextCollateralUsd < 0) {
    nextCollateralUsd = 0n;
  }
  let nextCollateralAmount = existingPosition ? existingPosition.collateralAmount - collateralDeltaAmount - payedRemainingCollateralAmount : 0n;
  ...
  const nextPendingImpactDeltaUsd = existingPosition?.pendingImpactUsd !== void 0 && proportionalPendingImpactDeltaUsd !== void 0 ? existingPosition.pendingImpactUsd - proportionalPendingImpactDeltaUsd : 0n;
  return {
    nextSizeUsd,
    nextCollateralUsd,
    nextLiqPrice,
...
export { estimateCollateralCost, getDecreasePositionAmounts, getIsFullClose, getMinCollateralUsdForLeverage, getNextPositionValuesForDecreaseTrade, getTotalFeesUsdForDecrease, payForCollateralCost };
