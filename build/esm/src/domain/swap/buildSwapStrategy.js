...
import 'viem/chains';
import { BOTANIX } from './chainIds.js';
...
var BBTC_ADDRESS = NATIVE_TOKEN_ADDRESS;
var PBTC_ADDRESS = getTokenBySymbol(BOTANIX, "PBTC").address;
var STBTC_ADDRESS = getTokenBySymbol(BOTANIX, "STBTC").address;
var AVAILABLE_BOTANIX_DEPOSIT_PAIRS = [
  {
    from: BBTC_ADDRESS,
    to: STBTC_ADDRESS
  },
...
var AVAILABLE_BOTANIX_WITHDRAW_PAIRS = [
  {
    from: STBTC_ADDRESS,
    to: PBTC_ADDRESS
  }
];
var getBotanixStakingExternalSwapPaths = ({
  fromTokenAddress
}) => {
  return [
...
var getAvailableExternalSwapPaths = ({
  chainId,
  fromTokenAddress
}) => {
  if (chainId === BOTANIX) {
...
function buildReverseSwapStrategy({
  ...
  if (approximateSwapPathStats) {
    const adjustedUsdIn = approximateSwapPathStats.usdOut > 0 ? bigMath2.mulDiv(
      approximateUsdIn,
      preferredUsdOut,
  ...
  if (suitableSwapPath) {
    ...
    const adjustedUsdIn = approximateSwapPathStatsForCombinedSwap.usdOut > 0 ? bigMath2.mulDiv(
      approximateUsdIn,
      preferredUsdOut,
