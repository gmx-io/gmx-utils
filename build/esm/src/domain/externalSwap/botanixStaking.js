import { defineChain, zeroAddress, encodeFunctionData } from 'viem';
import 'viem/chains';
...
// src/domain/externalSwap/botanixStaking.ts
...
var COEF_REDUCER = getBasisPoints(1n, 10000n);
var getBotanixStakingExternalSwapQuote = ({
  tokenInAddress,
  tokenOutAddress,
  amountIn,
  gasPrice,
  ...
  if (AVAILABLE_BOTANIX_DEPOSIT_PAIRS.some(
    ...
    const priceOut = bigMath2.mulDiv(
      priceIn,
      sharesPerAssetRate,
      BASIS_POINTS_DIVISOR_BIGINT
    );
    const usdIn = convertToUsd(amountIn, inTokenData.decimals, priceIn);
    const amountOut = amountIn > 0n ? bigMath2.mulDiv(
      amountIn,
      sharesPerAssetRate,
      BASIS_POINTS_DIVISOR_BIGINT
    ) - gasPrice : 0n;
