...
function usdToMarketTokenAmount(marketInfo, marketToken, usdValue) {
  ...
  return bigMath2.mulDiv(supply, usdValue, poolValue);
}
function marketTokenAmountToUsd(marketInfo, marketToken, amount) {
  const supply = marketToken.totalSupply;
  const poolValue = marketInfo.poolValueMax;
  const price = supply == 0n ? expandDecimals(1, USD_DECIMALS) : bigMath2.mulDiv(
    poolValue,
  ...
  if (strategy === "byCollaterals") {
    ...
    if (values.longTokenUsd > 0) {
      ...
      values.marketTokenAmount += getMarketTokenAmountByCollateral({
        ...
        priceImpactDeltaUsd: bigMath2.mulDiv(
    ...
    if (p.forShift) {
      ...
      values.longTokenUsd = bigMath2.mulDiv(
        values.marketTokenUsd,
        longPoolUsd,
        totalPoolUsd
      );
      values.shortTokenUsd = bigMath2.mulDiv(
        values.marketTokenUsd,
        shortPoolUsd,
        totalPoolUsd
      );
    ...
    if (totalDepositUsd > 0) {
      values.longTokenUsd = values.longTokenUsd + bigMath2.mulDiv(totalFee, values.longTokenUsd, totalDepositUsd);
      values.shortTokenUsd = values.shortTokenUsd + bigMath2.mulDiv(totalFee, values.shortTokenUsd, totalDepositUsd);
      totalDepositUsd = values.longTokenUsd + values.shortTokenUsd;
      if (values.swapPriceImpactDeltaUsd < 0 && totalDepositUsd > 0) {
        values.longTokenUsd = values.longTokenUsd + bigMath2.mulDiv(
          -values.swapPriceImpactDeltaUsd,
          values.longTokenUsd,
          totalDepositUsd
        );
        values.shortTokenUsd = values.shortTokenUsd + bigMath2.mulDiv(
          -values.swapPriceImpactDeltaUsd,
          values.shortTokenUsd,
          totalDepositUsd
        );
