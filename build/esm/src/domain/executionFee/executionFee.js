...
import 'viem/chains';
import { BOTANIX, ARBITRUM as ARBITRUM$1, AVALANCHE as AVALANCHE$1, AVALANCHE_FUJI as AVALANCHE_FUJI$1, ARBITRUM_SEPOLIA as ARBITRUM_SEPOLIA$1 } from './chainIds.js';
...
var CONTRACTS_CHAIN_IDS = [ARBITRUM$1, AVALANCHE$1, BOTANIX];
...
function convertBetweenTokens(tokenAmount, fromToken, toToken, maximize) {
  ...
  const fromPrice = fromToken.prices.minPrice;
  const toPrice = toToken.prices.maxPrice;
...
function getExecutionFee(chainId, gasLimits, tokensData, estimatedGasLimit, gasPrice, oraclePriceCount, numberOfParts) {
  ...
  if (minGasCost !== void 0) {
    feeTokenAmountPerExecution = bigMath2.max(
...
function estimateExecuteDecreaseOrderGasLimit(gasLimits, order) {
  ...
  if (order.decreaseSwapType !== 0 /* NoSwap */) {
    swapsCount += 1n;
  }
