...
import { TradeType, TradeMode, OrderType } from './types.js';
...
var getTradeFlags = (tradeType, tradeMode) => {
...
function getTradeFees(p) {
  ...
  const swapFees = !externalSwapQuote && initialCollateralUsd > 0 ? swapSteps.map((step) => ({
    ...
    precisePercentage: step.usdIn != 0n ? bigMath2.mulDiv(step.swapFeeUsd * -1n, PRECISION, step.usdIn) : 0n
  })) : void 0;
  const externalSwapFee = initialCollateralUsd > 0 && externalSwapQuote ? {
    ...
    precisePercentage: externalSwapQuote.usdIn != 0n ? bigMath2.mulDiv(
      externalSwapQuote.feesUsd * -1n,
      PRECISION,
      externalSwapQuote.usdIn
    ) : 0n
  ...
  const positionCollateralPriceImpact = getFeeItem(
    ...
    bigMath2.abs(collateralDeltaBasis)
  );
